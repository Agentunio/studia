const sourceNames = {
  arkusz: "z arkusza (1:1)",
  opracowanie: "opracowanie (nie 1:1 z arkusza)",
  pdf: "PDF / wykład"
};
...
function sourceTagClass(source) {
  if (source === "pdf") return "tag-ai";
  if (source === "opracowanie") return "tag-opracowanie";
  return "tag-sheet";
}

function sourceLabel(item) {
  return item.sourceRef || sourceNames[item.source] || item.source;
}
...
const checkResults = {};
const userAnswers = {};
const progressKey = "bazyDanychQuizProgress";
...
function fillFlashcardFilters() {
  ...
  elements.flashcardSourceSelect.innerHTML = sources
    .map((source) => {
      const label = source === "all" ? "Wszystkie" : sourceNames[source] || source;
...
function renderFlashcards() {
  ...
  elements.flashcardBack.textContent = state.flashcardFlipped ? `Źródło: ${sourceLabel(current)}` : "";
  ...
  elements.flashcardList.innerHTML = filtered
    .map(
      (card, index) => `
        ...
        >
          <span>${escapeHtml(card.category)} · ${escapeHtml(sourceLabel(card))}</span>
...
function getFilteredFlashcards() {
  return state.flashcardOrder
    ...
    .filter(Boolean)
    .filter((card) => {
      const text = [card.front, card.back, card.category, card.source, card.sourceRef].join(" ").toLowerCase();
    ...
    <article class="question-card exam-question" data-question-id="${escapeAttribute(question.id)}">
      <div class="question-main">
        <div class="meta">
          ...
          <span class="tag ${sourceClass}">${escapeHtml(sourceLabel(question))}</span>
